...
try:
    feed = pickle.load( open( "feed.p", "rb" ))
except (OSError, IOError) as e:
	print "no pickle file found, downloading data..."
	...
	pickle.dump(feed, open( "feed.p", "wb" ))
...
for post in feed:
	...
	if 'image_versions2' in post:
		...
		post_image=post['carousel_media'][0]['image_versions2']['candidates'][0]['url']
	sep='\n.\n.'
	...
	post_name=time.strftime('%Y-%m-%d', time.localtime(post_time))+'-'+url_string.lower()+".png"#".md"

	with open('/Users/lewisgavin/beermeupplease/images/'+post_name,'wb') as f:
	    f.write(urllib2.urlopen(post_image).read())
	    f.close()

	#f = open('temp/_posts/'+post_name, "w+")
	#f.write('---\nlayout: post\nauthor: Lewis Gavin\n---\n\n')
